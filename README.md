...
- **Revealed Content Used to Be Fully Public — Now Prototyped as Encrypted**: The deployed contract posts the seller's report in the open at reveal time, so technically anyone reading that transaction can read it for free, not just the buyer who paid for it. A prototype on the [`encrypted-reveal-prototype`](https://github.com/thespiritpilot/btech/tree/encrypted-reveal-prototype) branch fixes this for better security: the seller now encrypts the report to the buyer's own public key (ECIES), so only the buyer's private key can decrypt it — verified end-to-end with a passing test suite (`ScoutMarketEncrypted.sol` + `test/ScoutMarketEncrypted.test.js`, 24/24 tests green). Not merged into the live deployment, since it changes how disputes get verified — the contract can no longer check the hash itself at reveal time, so that check moves into the dispute path instead — and wasn't tested enough before the deadline to replace what's already deployed and verified on Sepolia.
